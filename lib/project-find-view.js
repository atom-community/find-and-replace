...
class ProjectFindView {
  ...
  handleEvents() {
    ...
    const updateInterfaceForSearching = () => {
      this.setInfoMessage('Searching...');
    };

    const updateInterfaceForResults = results => {
  ...
  confirm() {
    ...
    const searchPromise = this.search({onlyRunIfChanged: this.onlyRunIfChanged});
    this.onlyRunIfChanged = true;
  ...
  async search(options) {
    ...
    const {onlyRunIfActive, onlyRunIfChanged} = options;
  ...
  showResultPane() {
    ...
    const openDirection = atom.config.get('find-and-replace.projectSearchResultsPaneSplitDirection');
