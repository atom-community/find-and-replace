...
describe("FindView", () => {
  ...
  describe("when find-and-replace:show is triggered", () => {
    it("attaches FindView to the root view", async () => {
      await atom.commands.dispatch(editorView, "find-and-replace:show");
    ...
    it('selects the text to find when the panel is re-shown', async () => {
      ...
      await atom.commands.dispatch(findEditor.element, "core:confirm");
      await atom.commands.dispatch(document.activeElement, "core:cancel");
      await atom.commands.dispatch(editorView, "find-and-replace:show");
  ...
  describe("when find-and-replace:toggle is triggered", () => {
    it("toggles the visibility of the FindView", async () => {
      await atom.commands.dispatch(workspaceElement, "find-and-replace:toggle");
      await activationPromise;
  ...
  describe("when the find-view is focused and window:focus-next-pane is triggered", () => {
    it("attaches FindView to the root view", async () => {
      ...
      await atom.commands.dispatch(findView.findEditor.element, "window:focus-next-pane");
  ...
  describe("find-and-replace:show-replace", () => {
    it("focuses the replace editor", async () => {
      await atom.commands.dispatch(editorView, "find-and-replace:show-replace");
      await activationPromise;
    ...
    it("does not escape the text when the regex option is enabled", async () => {
      ...
      await atom.commands.dispatch(editorView, "find-and-replace:show");
      await atom.commands.dispatch(editorView, "find-and-replace:toggle-regex-option");
      await atom.commands.dispatch(editorView, "find-and-replace:show-replace");
      await activationPromise;
  ...
  describe("when find-and-replace:clear-history is triggered", () => {
    it("clears the find and replace histories", async () => {
      ...
      await atom.commands.dispatch(editorView, "find-and-replace:clear-history");
      await atom.commands.dispatch(findView.findEditor.element, "core:move-up");
      expect(findView.findEditor.getText()).toBe("");

      await atom.commands.dispatch(findView.replaceEditor.element, "core:move-up");
  ...
  describe("core:cancel", () => {
    beforeEach(async () => {
      ...
      findView.findEditor.setText("items");
      await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
    ...
    describe("when core:cancel is triggered on the find view", () => {
      ...
      it("removes highlighted matches", async () => {
    ...
    describe("when core:cancel is triggered on an empty pane", () => {
      it("hides the find panel", async () => {
        const paneElement = atom.views.getView(atom.workspace.getCenter().getActivePane());
        paneElement.focus();
        await atom.commands.dispatch(paneElement, "core:cancel");
        expect(getFindAtomPanel()).not.toBeVisible();
      });
    });
    ...
    describe("when core:cancel is triggered on an editor", () => {
      it("detaches from the workspace view", async () => {
        atom.workspace.open();
        await atom.commands.dispatch(editorView, "core:cancel");
        expect(getFindAtomPanel()).not.toBeVisible();
      });
    });

    describe("when core:cancel is triggered on a mini editor", () => {
      it("leaves the find view attached", async () => {
        ...
        miniEditor.focus();
        await atom.commands.dispatch(miniEditor, "core:cancel");
        expect(getFindAtomPanel()).toBeVisible();
      });
    });
  });
  ...
  describe("finding", () => {
    ...
    describe("when find-and-replace:confirm is triggered", () => {
      it("runs a search", async () => {
        ...
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:confirm");
    ...
    describe("when no results are found", () => {
      it("adds a .has-no-results class", async () => {
        findView.findEditor.setText("notinthefile");
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:confirm");
    ...
    describe("when results are found", () => {
      it("adds a .has-results class", async () => {
        findView.findEditor.setText("items");
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:confirm");
    ...
    describe("when the find string contains an escaped char", () => {
      ...
      describe("when regex search is enabled", () => {
        beforeEach(async () => {
          await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-regex-option");
        });
        ...
        it("finds a newline", async () => {
          findView.findEditor.setText("\\n");
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
          expect(editor.getSelectedBufferRange()).toEqual([[0, 1], [1, 0]]);
        });

        it("finds a tab character", async () => {
          findView.findEditor.setText("\\t");
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
      ...
      describe("when regex search is disabled", () => {
        it("finds the literal backslash t", async () => {
          findView.findEditor.setText("\\t");
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        ...
        it("finds two backslashes", async () => {
          findView.findEditor.setText('\\\\');
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
          expect(editor.getSelectedBufferRange()).toEqual([[1, 2], [1, 4]]);
        });

        it("doesn't find when escaped", async () => {
          findView.findEditor.setText("\\\\t");
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
    ...
    describe("when whole-word search is enabled", () => {
      beforeEach(async () => {
        editor.setText("-----\nswhole-wordy\nwhole-word\nword\nwhole-swords");
        editor.setCursorBufferPosition([0, 0]);
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-whole-word-option");
      });

      it("finds the whole words", async () => {
        findView.findEditor.setText("word");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(editor.getSelectedBufferRange()).toEqual([[2, 6], [2, 10]]);
      });

      it("doesn't highlight the search inside words", async () => {
        findView.findEditor.setText("word");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
    ...
    it("doesn't change the selection, beeps if there are no matches and keeps focus on the find view", async () => {
      editor.setCursorBufferPosition([2, 0]);
    ...
    describe("updating the replace button enablement", () => {
      it("enables the replace buttons when are search results", async () => {
    ...
    describe("updating the descriptionLabel", () => {
      it("properly updates the info message", async () => {
        findView.findEditor.setText("item");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
      ...
      describe("when there is an error", () => {
        describe("when the regex search string is invalid", async () => {
          beforeEach(async () => {
            await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-regex-option");
          ...
          it("will be reset when there is no longer an error", async () => {
            expect(findView.refs.descriptionLabel).toHaveClass("text-error");
    ...
    it("selects the first match following the cursor", async () => {
    ...
    it("selects the next match when the 'find-and-replace:find-next' event is triggered and correctly focuses the editor", async () => {
      expect(findView.element).toHaveFocus();
      await atom.commands.dispatch(editorView, "find-and-replace:find-next");
      expect(findView.refs.resultCounter.textContent).toEqual("3 of 6");
      expect(editor.getSelectedBufferRange()).toEqual([[2, 34], [2, 39]]);
      expect(editorView).toHaveFocus();
    });

    it("selects the previous match before the cursor when the 'find-and-replace:show-previous' event is triggered", async () => {
      expect(findView.refs.resultCounter.textContent).toEqual("2 of 6");
      expect(editor.getSelectedBufferRange()).toEqual([[2, 8], [2, 13]]);
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:show-previous");
      expect(findView.refs.resultCounter.textContent).toEqual("1 of 6");
      expect(editor.getSelectedBufferRange()).toEqual([[1, 22], [1, 27]]);
      expect(findView.findEditor.element).toHaveFocus();
    });
    ...
    it("will re-run search if 'find-and-replace:find-next' is triggered after changing the findEditor's text", async () => {
      findView.findEditor.setText("sort");
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:find-next");
      expect(findView.refs.resultCounter.textContent).toEqual("3 of 5");
      expect(editor.getSelectedBufferRange()).toEqual([[8, 11], [8, 15]]);
    });

    it("'find-and-replace:find-next' adds to the findEditor's history", async () => {
      findView.findEditor.setText("sort");
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:find-next");
      expect(findView.refs.resultCounter.textContent).toEqual("3 of 5");
    ...
    it("selects the previous match when the 'find-and-replace:find-previous' event is triggered and correctly focuses the editor", async () => {
      expect(findView.element).toHaveFocus();

      await atom.commands.dispatch(editorView, "find-and-replace:find-previous");
      expect(findView.refs.resultCounter.textContent).toEqual("1 of 6");
      expect(editor.getSelectedBufferRange()).toEqual([[1, 27], [1, 22]]);
      expect(editorView).toHaveFocus();
    });

    it("will re-run search if 'find-and-replace:find-previous' is triggered after changing the findEditor's text", async () => {
      findView.findEditor.setText("sort");
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:find-previous");
      expect(findView.refs.resultCounter.textContent).toEqual("2 of 5");
      expect(editor.getSelectedBufferRange()).toEqual([[1, 6], [1, 10]]);
    });

    it("selects all matches when 'find-and-replace:find-all' is triggered and correctly focuses the editor", async () => {
      expect(findView.element).toHaveFocus();
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:find-all");
    ...
    it("will re-run search if 'find-and-replace:find-all' is triggered after changing the findEditor's text", async () => {
      findView.findEditor.setText("sort");
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:find-all");
    ...
    it("shows an icon when search wraps around and the editor scrolls", async () => {
    ...
    it("does not show the wrap icon when the editor does not scroll", async () => {
    ...
    it("allows searching for dashes in combination with non-ascii characters (regression)", async () => {
    ...
    describe("when find-and-replace:use-selection-as-find-pattern is triggered", () => {
      it("places the selected text into the find editor", async () => {
        editor.setSelectedBufferRange([[1, 6], [1, 10]]);
        await atom.commands.dispatch(workspaceElement, "find-and-replace:use-selection-as-find-pattern");
        expect(findView.findEditor.getText()).toBe("sort");
        expect(editor.getSelectedBufferRange()).toEqual([[1, 6], [1, 10]]);

        await atom.commands.dispatch(workspaceElement, "find-and-replace:find-next");
        expect(editor.getSelectedBufferRange()).toEqual([[8, 11], [8, 15]]);
      ...
      it("places the word under the cursor into the find editor", async () => {
      ...
      it("places the previously selected text into the find editor if no selection", async () => {
        editor.setSelectedBufferRange([[1, 6], [1, 10]]);
        await atom.commands.dispatch(workspaceElement, "find-and-replace:use-selection-as-find-pattern");
        expect(findView.findEditor.getText()).toBe("sort");
      ...
      it("searches for the amount of results", async () => {
    ...
    describe("when find-and-replace:use-selection-as-replace-pattern is triggered", () => {
      it("places the selected text into the replace editor", async () => {
        editor.setSelectedBufferRange([[3, 8], [3, 13]]);
        await atom.commands.dispatch(workspaceElement, 'find-and-replace:use-selection-as-replace-pattern');
        ...
        await atom.commands.dispatch(workspaceElement, 'find-and-replace:replace-next');
        expect(editor.getTextInBufferRange([[8, 11], [8, 16]])).toEqual('pivot');
        expect(editor.getSelectedBufferRange()).toEqual([[8, 44], [8, 48]]);
        expect(editor.getTextInBufferRange(editor.getSelectedBufferRange())).toEqual('sort');
      });

      it("places the word under the cursor into the replace editor", async () => {
      ...
      it("places the previously selected text into the replace editor if no selection", async () => {
        editor.setSelectedBufferRange([[1, 6], [1, 10]]);
        await atom.commands.dispatch(workspaceElement, 'find-and-replace:use-selection-as-replace-pattern');
      ...
      it("places selected text into the replace editor and escapes it when Regex is enabled", async () => {
        await atom.commands.dispatch(findView.replaceEditor.element, 'find-and-replace:toggle-regex-option');
        editor.setSelectedBufferRange([[6, 6], [6, 65]]);
        await atom.commands.dispatch(workspaceElement, 'find-and-replace:use-selection-as-replace-pattern');
    ...
    describe("when find-and-replace:find-next-selected is triggered", async () => {
      it("places the selected text into the find editor and finds the next occurrence", async () => {
        editor.setSelectedBufferRange([[0, 9], [0, 13]]);
        await atom.commands.dispatch(workspaceElement, "find-and-replace:find-next-selected");
        expect(findView.findEditor.getText()).toBe("sort");
        expect(editor.getSelectedBufferRange()).toEqual([[1, 6], [1, 10]]);
      });

      it("places the word under the cursor into the find editor and finds the next occurrence", async () => {
        editor.setSelectedBufferRange([[1, 8], [1, 8]]);
        await atom.commands.dispatch(workspaceElement, "find-and-replace:find-next-selected");
        expect(findView.findEditor.getText()).toBe("sort");
        expect(editor.getSelectedBufferRange()).toEqual([[8, 11], [8, 15]]);
    ...
    describe("when find-and-replace:find-previous-selected is triggered", () => {
      it("places the selected text into the find editor and finds the previous occurrence ", async () => {
        editor.setSelectedBufferRange([[0, 9], [0, 13]]);
        await atom.commands.dispatch(workspaceElement, "find-and-replace:find-previous-selected");
        expect(findView.findEditor.getText()).toBe("sort");
        expect(editor.getSelectedBufferRange()).toEqual([[11, 9], [11, 13]]);
      });

      it("places the word under the cursor into the find editor and finds the previous occurrence", async () => {
    ...
    it("does not highlight the found text when the find view is hidden", async () => {
      await atom.commands.dispatch(findView.findEditor.element, "core:cancel");
      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:find-next");
    });
    ...
    describe("when the active pane item changes", () => {
      ...
      describe("when all active pane items are closed", () => {
        it("updates the result count", async () => {
          await atom.commands.dispatch(editorView, "core:close");
      ...
      describe("when a new editor is activated on a different pane", () => {
        ...
        it("will still highlight results after the split pane has been destroyed", async () => {
          ...
          await atom.commands.dispatch(editor.element, "core:close");
          editorView.focus();
    ...
    describe("when the buffer contents change", () => {
      ...
      it("does not beep if no matches were found", async () => {
        editor.setCursorBufferPosition([2, 0]);
        findView.findEditor.setText("notinthefilebro");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        atom.beep.reset();
    ...
    describe("when in current selection is toggled", () => {
      ...
      it("toggles find within a selection via an event and only finds matches within the selection", async () => {
        findView.findEditor.setText("items");
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-selection-option");
      ...
      describe("when there is no selection", () => {
        ...
        it("toggles find within a selection via an event", async () => {
          findView.findEditor.setText("items");
          await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-selection-option");
          expect(editor.getSelectedBufferRange()).toEqual([[1, 22], [1, 27]]);
          expect(findView.refs.resultCounter.textContent).toEqual("1 of 6");
    ...
    describe("when regex is toggled", () => {
      ...
      it("re-runs the search using the new find text when toggled", async () => {
      ...
      it("matches astral-plane unicode characters with .", async () => {
    ...
    describe("when whole-word is toggled", () => {
      it("toggles whole-word via an event and finds text matching the pattern", async () => {
        editor.setCursorBufferPosition([0, 0]);
        findView.findEditor.setText("sort");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
      ...
      it("toggles whole-word via a button and finds text matching the pattern", async () => {
        editor.setCursorBufferPosition([0, 0]);
        findView.findEditor.setText("sort");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(editor.getSelectedBufferRange()).toEqual([[0, 9], [0, 13]]);
    ...
    describe("when case sensitivity is toggled", () => {
      ...
      it("toggles case sensitivity via an event and finds text matching the pattern", async () => {
        findView.findEditor.setText("WORDs");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(editor.getSelectedBufferRange()).toEqual([[1, 0], [1, 5]]);

        editor.setCursorBufferPosition([0, 0]);
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-case-option");
        expect(editor.getSelectedBufferRange()).toEqual([[2, 0], [2, 5]]);
      });

      it("toggles case sensitivity via a button and finds text matching the pattern", async () => {
        findView.findEditor.setText("WORDs");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(editor.getSelectedBufferRange()).toEqual([[1, 0], [1, 5]]);
      ...
      it("finds unicode characters with case folding", async () => {
        if (!editor.getBuffer().hasAstral) {
    ...
    describe("highlighting search results", () => {
      ...
      it("only highlights matches", async () => {
        expect(getResultDecorations(editor, "find-result")).toHaveLength(5);
        findView.findEditor.setText("notinthefilebro");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(getResultDecorations(editor, "find-result")).toHaveLength(0);
      ...
      it("adds a class to the current match indicating it is the current match", async () => {
    ...
    describe("when user types in the find editor", () => {
      ...
      it("doesn't live search on a regex that matches empty string", async () => {
        expect(findView.refs.descriptionLabel.textContent).toContain("6 results");
    ...
    describe("when another find is called", () => {
      it("clears existing markers for another search", async () => {
        findView.findEditor.setText("notinthefile");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(getResultDecorations(editor, "find-result")).toHaveLength(0);
      });

      it("clears existing markers for an empty search", async () => {
        findView.findEditor.setText("");
        await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
        expect(getResultDecorations(editor, "find-result")).toHaveLength(0);
      });
  ...
  describe("replacing", () => {
    ...
    describe("when the find string is empty", () => {
      it("beeps", async () => {
        findView.findEditor.setText("");
        await atom.commands.dispatch(findView.replaceEditor.element, "core:confirm");
        expect(atom.beep).toHaveBeenCalled();
      });
    });
    ...
    describe("when the replacement string contains an escaped char", () => {
      describe("when the regex option is chosen", () => {
        ...
        it("inserts newlines and tabs", async () => {
      ...
      describe("when in normal mode", () => {
        it("inserts backslash n and t", async () => {
          findView.replaceEditor.setText("\\t\\n");
          await atom.commands.dispatch(findView.replaceEditor.element, "core:confirm");
          expect(editor.getText()).toMatch(/\\t\\n/);
        });

        it("inserts carriage returns", async () => {
    ...
    describe("replace next", () => {
      describe("when core:confirm is triggered", () => {
        it("replaces the match after the cursor and selects the next match", async () => {
          await atom.commands.dispatch(findView.replaceEditor.element, "core:confirm");
          expect(findView.refs.resultCounter.textContent).toEqual("2 of 5");
        ...
        it("replaceEditor maintains focus after core:confirm is run", async () => {
          findView.replaceEditor.element.focus();
          await atom.commands.dispatch(findView.replaceEditor.element, "core:confirm");
          expect(findView.replaceEditor.element).toHaveFocus();
        });

        it("replaces the _current_ match and selects the next match", async () => {
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
          editor.setSelectedBufferRange([[2, 8], [2, 13]]);
          expect(findView.refs.resultCounter.textContent).toEqual("2 of 6");
      ...
      describe("when the 'find-and-replace:replace-next' event is triggered", () => {
        it("replaces the match after the cursor and selects the next match", async () => {
          await atom.commands.dispatch(editorView, "find-and-replace:replace-next");
          expect(findView.refs.resultCounter.textContent).toEqual("2 of 5");
          expect(editor.lineTextForBufferRow(2)).toBe("    if (cats.length <= 1) return items;");
          expect(editor.getSelectedBufferRange()).toEqual([[2, 33], [2, 38]]);
        });
    ...
    describe("replace previous", () => {
      describe("when command is triggered", () => {
        it("replaces the match after the cursor and selects the previous match", async () => {
          await atom.commands.dispatch(findView.findEditor.element, "core:confirm");
          await atom.commands.dispatch(findView.element, "find-and-replace:replace-previous");
    ...
    describe("replace all", () => {
      ...
      describe("when the 'find-and-replace:replace-all' event is triggered", () => {
        it("replaces all matched text", async () => {
          await atom.commands.dispatch(editorView, "find-and-replace:replace-all");
          expect(findView.refs.resultCounter.textContent).toEqual("no results");
          expect(editor.getText()).not.toMatch(/items/);
          expect(editor.getText().match(/\bcats\b/g)).toHaveLength(6);
          expect(editor.getSelectedBufferRange()).toEqual([[2, 0], [2, 0]]);
    ...
    describe("replacement patterns", () => {
      describe("when the regex option is true", () => {
        it("replaces $1, $2, etc... with substring matches", async () => {
          await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-regex-option");
          findView.findEditor.setText("(items)([\\.;])");
      ...
      describe("when the regex option is false", () => {
        it("replaces the matches with without any regex subsitions", async () => {
          findView.findEditor.setText("items");
          findView.replaceEditor.setText("$&cats");
          await atom.commands.dispatch(editorView, "find-and-replace:replace-all");
          expect(editor.getText()).not.toMatch(/items/);
  ...
  describe("history", () => {
    ...
    describe("when there is no history", () => {
      it("retains unsearched text", async () => {
        ...
        await atom.commands.dispatch(findView.findEditor.element, "core:move-down");
    ...
    describe("when there is history", () => {
      ...
      it("can navigate the entire history stack", async () => {
      ...
      it("retains the current unsearched text", async () => {
        const text = "something I want to search for but havent yet";
        findView.findEditor.setText(text);
        await atom.commands.dispatch(findView.findEditor.element, "core:move-up");
        expect(findView.findEditor.getText()).toEqual("three");
      ...
      it("adds confirmed patterns to the history", async () => {
  ...
  describe("panel focus", () => {
    ...
    it("moves focus between editors with find-and-replace:focus-next", async () => {
      findView.findEditor.element.focus();
      expect(findView.findEditor.element).toHaveClass("is-focused");
      expect(findView.replaceEditor).not.toHaveClass("is-focused");

      await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:focus-next");
      ...
      await atom.commands.dispatch(findView.replaceEditor.element, "find-and-replace:focus-next");
      expect(findView.findEditor.element).toHaveClass("is-focused");
  ...
  describe("when language-javascript is active", () => {
    ...
    describe("when panel is active", async () => {
      beforeEach(async () => {
        await atom.commands.dispatch(editorView, "find-and-replace:show");
        await activationPromise;
      });
      ...
      it("uses regexp grammar when in regex mode and clears the regexp grammar when regex is disabled", async () => {
        await atom.commands.dispatch(findView.findEditor.element, "find-and-replace:toggle-regex-option");
        expect(findView.model.getFindOptions().useRegex).toBe(true);
        expect(findView.findEditor.getGrammar().scopeName).toBe("source.js.regexp");
        expect(findView.replaceEditor.getGrammar().scopeName).toBe("source.js.regexp.replacement");
